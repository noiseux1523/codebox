...
while (ns < nscal && fcount <= budget && failc < imfil_maxfail ...
       && stop_now == 0  && fval > imfil_target) % MAIN LOOP 
    ns=ns+1; h=dscal(ns); 
%
...
if imfil_noise_aware > 0 || fcount == 0 || imfil_scale_aware > 0
    [funs,iff,icf,noise_val]=feval(f,x,h,core_data); % PREMIERE EXECUTION
...
%
    ...
    fcount=fcount+icf; % ICF = 1, car on a caculé une fois la fonction et que le cout est de 1 compte par run de f.
...
% ICI ON CALCULE N POINTS A MOINS QUE IL Y EN AIT DES IRRÉALISABLES
...
%     dANS CETTE FONCTION LA, IL CALCULE LA FONCTION SUREMENT N FOIS
%
...
%    ICI ON CALCUL ENCORE DES FONCTIONS DANS LE CALCUL DU GRADIENT.
%
...
% Thisdir = hess\sgrad;
% function updates the record of the best point using the 
...
[best_value,best_value_f,best_point,icount,sgood,good_points,good_values,...
         good_dx,good_df,failed_points] = ...
           imfil_poll_stencil(x,f,dx,fc,bounds,core_data,h,complete_history,fc);
%
...
function [best_value,best_value_f,best_point,icount,sgood,...
      good_points,good_values,good_dx,good_df,failed_points] = ...
              imfil_poll_stencil(x,f,dx,fc,bounds,core_data,h,complete_history,current_best)
% IMFIL_POLL_STENCIL
...
switch ordo
    ...
    case 's'
        ...
        xp1 = ios(xp1,x,complete_history);
    case 'm'
        % Avec ordonnancement omniscient
        xp1 = iom(xp1,x,complete_history);
    case 'o'
...
if mf == 1 && lsq == 1
 disp('Your funcimfil_poll_stenciltion is scalar-valued but the least_squares option is on.'); 
